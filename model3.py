...
mean_loc = tf.reduce_mean(stack_of_loc, axis=1)
random_uniform = tf.random_uniform([1], 0, data_dimension)
choice = tf.math.ceil(random_uniform)
choice = tf.cast(choice, tf.int32)-1
loc = stack_of_loc[:, choice[0], :]
...
penalty_sum = 0.0
for j in range(data_dimension):
    penalty_sum += tf.square(stack_of_loc[:, j, :] - mean_loc)
penalty_sum = penalty_sum/data_dimension
penalty_sum = pen_param*tf.reduce_mean(penalty_sum, axis=0)
